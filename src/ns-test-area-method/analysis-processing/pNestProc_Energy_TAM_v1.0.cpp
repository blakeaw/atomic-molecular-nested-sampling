...
///
//Input File Name
string filename = "Epoints_NS_CP_n200_ab2_rep1_comb.dat";
string filename2 = "zObservables_NS_CP_n200_ab2_rep1_comb.dat";
//Output File Name
string outfilename = "zNestOut_CP_n200_ab2_rep1.dat";
...
long double Hn[20000] = {0};
long double tex, junk;
int i = 0;
const unsigned long ii = 2600;
...
cout<<"read in "<<ii<<" enthalpy values."<<endl;
long double Obs[ii][6];
...
//	cout<<jjj<<" t1: "<<tex1<<" t2 "<<tex2<<" t3: "<<tex3<<" t4 "<<tex4<<" t5: "<<tex5<<" t6 "<<tex6<<endl;
...
//		lw = log( pow(0.75, n-1)-pow(0.75, n) )- (Beta*Hcurr);
//		cout<<"Temp"<<Temp<<" n "<<n<<" lw "<<lw<<endl;	
		Ws+=pow(2.0, -n);
		while(lw<Sl-lwmax){
			Zest *= exp(lwmax);
			He *= exp(lwmax);
...
//		Zunscale+=Wnunscale;
	...
	long double G = -kb*Temp*( log(Zest) + Sl+Sh - log(Ws));
	long double mu = G/N;
...
//	cout<<"obs1 "<<Obs1<<" obs3 "<<Obs3<<" obs4 "<<Obs4<<endl;
	outfile<<std::scientific << setiosflags(ios::fixed)<< setprecision(15)<<Temp<<" "<<Zest<<" "<<H<<" "<<gamma<<" "<<cvn<<" "<<G<<" "<<Obs4<<" "<<Obs1<<" "<<Obs3<<" "<<Obs2<<endl;
}
