...
int main(int argc, char *argv[])
{
	...
	long double Vmax = 90000.0;
	//Vmax*=15.0;
	const long double dA=0.005;
	//Box sizes
...
//	boxz/=(sca*sca);
	...
	long double vstepguess = 400.0/Press;
	...
	string rundescript = "NS_CP_n"+nds+"_ab9";
...
logfile<<"Initial Enthalpy "<<Ht1<<" Initial Energy: "<<Et1<<" Vinitial "<<Vinitial<<endl;
	...
	long double stepxl, stepyl, stepzl;
	stepxl=stepx;
	stepyl=stepy;
	stepzl=stepz;
  //Perform markov moves on the configuration
    for(unsigned int n = 0; n<MCitpp; ++n){
		...
		long double ran1,ran2,ran3;
		#pragma omp critical
		{
			ran1= dsfmt_genrand_close_open(&dsfmt);
			ran2= dsfmt_genrand_close_open(&dsfmt);
			ran3= dsfmt_genrand_close_open(&dsfmt);
		...
		if(n%Vint==0){
				...
				long double Vnew = Vcurr + (ran1-0.5)*vstep;
				while (Vnew<Vexc){
			...
			//	dV = Vnew - Vcurr;
				...
				if (vprob>ranprob){
					...
					bz=Tlocal.boxz*bscale;
					Tlocal.SetBox(bx, by, bz);
					long double Enew = TotalPairEnergyPeriodic(Tlocal);
			//	dE += Enew - Et1;
					Et1local = Enew;
					vdeftag = 1;
					stepxl = 0.5*Tlocal.boxx*pow(0.75, cfailcount);
					stepyl = 0.5*Tlocal.boxy*pow(0.75, cfailcount);
					stepzl = 0.5*Tlocal.boxz*pow(0.75, cfailcount);
					Vcurr = Vnew;
		...
		//Coordinate change		
			...
			movex = (ran1-0.5)*stepxl;
			movey = (ran2-0.5)*stepyl;
			movez = (ran3-0.5)*stepzl;
...
//			}
		...
		if(tagg == 1){
			...
			//	long double LD = (long double)(natoms)/Vcurr;
				long double LD = Tlocal.boxx*Tlocal.boxy;
				long double scale = sqrt(1.0+ ( dA/(Tlocal.boxx*Tlocal.boxy)));
...
long double PairEnergyO(const Atom& one, const Atom& old, const long double& ox, const long double& oy, const long double& oz, const long double& boxx, const long double& boxy, const long double& boxz){
	...
	eps = sqrt(one.epsilon*old.epsilon);
