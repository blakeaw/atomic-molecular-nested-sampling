...
logfile << " " << endl;
	...
	logfile<<"Constant Volume Nested Sampling of lennard jones system with TAM - hard spherical com boundary"<<endl;
...
//	RunningStats Rgstat;
	Histogram Eohist;
	Eohist.Initialize(Binwidth*10.0, Elow, Etop);
...
logfile << "Iteration " << ii << "......." << endl;
 if(ii>0){
	...
	obsoutfile<<Eavg<<" "<<Esqavg<<" "<<LDavg<<endl;
...
//			Tlocal.WrapCoordinates();
		...
		if(Et1local < Enest && bflag){
			
			//cout<<"trial move accepted. Eelocal was: "<<Eelocal<<" dE: "<<dE<<" bflag: "<<bflag<<" rdist "<<rdist<<endl;
			tagg=0;
			Eelocal=Et1local;
		//	cout<<"Eelocal is now: "<<Eelocal<<" Et1local: "<<Et1local<<" n "<<n<<endl;
//			cout<<"movex: "<<movex<<" movey: "<<movey<<" movez: "<<movez<<endl;
		//	cout<<"stepx: "<<stepx<<" stepy: "<<stepy<<" stepz: "<<stepz<<endl;		
		//	cout<<"origx: "<<origx<<" t.a.x: "<<Tlocal.atom[Arandom].x<<endl;
//			Tlocal.CalcCom();
//			Tlocal.ReCenterZero();
			if (Eelocal<Elowprev){
		...
		if(tagg == 1){
				...
				Et1local = Et1b;
			//	cout<<"Trial move failed: setting Et1local back to : "<<Et1b<<endl;
...
//				#pragma omp critical
//				{
//					cout<<" Pushing Eelocal: "<<Eelocal<<endl;
//					cout<<"Et1local is : "<<Et1local<<endl;
					//exit(0);
					Ehist.Push(Eelocal);
					Estat.Push(Eelocal);
