...
int main(int argc, char *argv[])
{
	...
	const unsigned int MCit = 16000000;
	...
	const unsigned Eqit = 8000000;
	// Step interval to collect data after equilibration
	const unsigned Cit = natoms*2;
	long double Temp = 0.3;
	//Box sizes
//	long double boxx = 5.0/1.587;//*1.1;
//	long double boxy = 5.0/1.587;//*1.1;
//	long double boxz = 10.0/1.587;///(1.1*1.1);
	long double rdist = 6.60;
	// Test area scaling value
	long double scale = 0.25001;
//	long double abox = (boxx+boxy+boxz)/3.0;
	//Translation step
	const long double bsfac = 0.030000;
 	
    long double stepx = rdist*bsfac;
	long double stepy = rdist*bsfac;
	long double stepz = rdist*bsfac;
	
	long double Etop = 100.0;
	long double Elow = -100.0;
	long double Binwidth = 0.01;


		//run descriptor
	string nds = itos(natoms);
	string rundescript = "MC_n"+nds+"_c0_T0.3_a";
...
//	ofstream stepoutfile("zStepOut_int_test_a1.dat");	
	...
	unsigned int ConfigsOutFreq = ((MCitpp-Eqit))/100;
...
logfile << " " << endl;
	...
	logfile<<"Constant Volume MC Monte Carlo of lennard jones system with TAM - hard spherical com boundary"<<endl;
...
//	logfile<<" boxx: "<<boxx<<" boxy: "<<boxy<<" boxz: "<<boxz<<endl;
	logfile<<"radial com distance constraint: "<<rdist<<endl;
...
//	RunningStats Rgstat;
	Histogram Eohist;
	Eohist.Initialize(Binwidth, Elow, Etop);
...
do
{
	...
	{
		long double rx = (dsfmt_genrand_close_open(&dsfmt)-0.5)*0.5*rdist;
		long double ry = (dsfmt_genrand_close_open(&dsfmt)-0.5)*0.5*rdist;
		long double rz = (dsfmt_genrand_close_open(&dsfmt)-0.5)*0.5*rdist;
		trial1.atom[i].x = rx;
		trial1.atom[i].y = ry;
		trial1.atom[i].z = rz;
		bool flag=0;
...
stepx = bsfac*rdist*pow(0.90, cfailcount);
stepy = bsfac*rdist*pow(0.90, cfailcount);
stepz = bsfac*rdist*pow(0.90, cfailcount);
...
//			Tlocal.ReCenterZero();
		...
		if(tagg == 1){
			...
			if (n==Eqit){
				 ...
				 #pragma omp critical
				 {
					cout<<" local acceptance ratio after Eqit is: "<<locrat<<endl;
				 }
				 if (locrat<0.33){
				 	#pragma omp critical
					{
...
//					dUcstat.Push(dU*dU*dU);
	   ...
	   if(n>Eqit && (n-Eqit)%(ConfigsOutFreq)==0) {
			Config Outc(Tlocal);
			//Outc.SetBox(boxx, boxy, boxz);
			Outc.CalcCom();
			Outc.ReCenterZero();
			#pragma omp flush(coutfile)
			#pragma omp critical 
			{
				...
				for(unsigned long hg = 0; hg<Tlocal.natoms; ++hg){
					coutfile << setiosflags(ios::fixed) << setprecision(15) << "lj " << Outc.atom[hg].x << " " << Outc.atom[hg].y << " " << Outc.atom[hg].z << endl; 
				}
			}
		//cout<<" -----------------------------"<<endl;
...
//long double gamma = dUavg-0.5*Beta*( dUsavg - dUavg*dUavg);
//Surface tension
//long double dA = boxx*boxy*(scale*scale - 1.0);
//cout<<"bx "<<boxx<<" by "<<boxy<<" scale "<<scale<<" dA "<<dA<<endl;
//logfile<<"bx "<<boxx<<" by "<<boxy<<" scale "<<scale<<" dA "<<dA<<endl;
//gamma += (1.0/6.0)*Beta*Beta*( dUcavg - 3.0*dUsavg*dUavg+2.0*dUcavg*dUcavg);
//gamma/=dA;
//gamma/=2.0;
...
long double cv = (Esqavg-(Eavg*Eavg))/(Temp*Temp);
obsoutfile<<Eavg<<" "<<Esqavg<<" "<<LDavg<<" "<<cv<<endl;
Eohist.Normalize();
Eohist.DumpNorm(eoutfile);
