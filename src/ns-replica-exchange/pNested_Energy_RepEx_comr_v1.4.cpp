...
int main(int argc, char *argv[])
{
	...
	const unsigned short nprocs = 20;
	...
	const unsigned Cit = natoms*20;
	long double NSfrac = 0.5;
	// Swap probability
	const long double Pex = 0.01;
	//Box sizes
	long double boxx = 3.2;//*1.1;
	long double boxy = 3.2;//*1.1;
	long double boxz = 3.2;///(1.1*1.1);
	...
	string rundescript = "NS_comr_n"+nds+"_a10";
	//coordinates to read in
	string infilename = "LJ38_GlobalMin.txt";
	ifstream infile;
...
long double EGlobMin = TotalEnergy(GlobMin);
long double Q4GlobMin = ClusterQ4(GlobMin);
cout<<"Global Minimum Energy: "<<EGlobMin<<" with Q4 "<<Q4GlobMin<<endl;
logfile<<"Global Minimum Energy: "<<EGlobMin<<" with Q4 "<<Q4GlobMin<<endl;
...
cout<<"Averag Q4: "<<LDavg<<endl;
logfile<<"Average Q4: "<<LDavg<<endl;
...
for (unsigned int np = 0; np < nprocs; ++np)
{
	long double Q4min = ClusterQ4(lgmin[np]);
	cout<<"Walker "<<np<<" Enest "<<Enestloc[np]<<" Enmin : "<<Enmin[np]<<" Q4min: "<<Q4min<<endl;
	logfile<<"Walker "<<np<<" Enest "<<Enestloc[np]<<" Enmin : "<<Enmin[np]<<" Q4min: "<<Q4min<<endl;
...
logfile<<"Initial Energy: "<<Et1<<endl;
	...
	unsigned npush[nprocs]={0};
	unsigned nexatt[nprocs-1]={0};
	unsigned nexsuc[nprocs-1]={0};
...
//	   }	
	...
	++n;
		...
		if (ii>0 && tid<nprocs-1 && ran[4]<Pex){
	...
	//number of points contributed to the histogram
	for (unsigned int np = 0; np < nprocs; ++np)
	{
		long double fraction = (long double)(npush[np])/(long double)(Nsamples);
		cout<<"Walker "<<np<<" contributed "<<npush[np]<<" samples which is "<<fraction<<" of total."<<endl;
		logfile<<"Walker "<<np<<" contributed "<<npush[np]<<" samples which is "<<fraction<<" of total."<<endl;
	}	
	for (unsigned int np = 0; np < nprocs-1; ++np)
	{
		long double fraction = (long double)(nexsuc[np])/(long double)(nexatt[np]);
		cout<<"Walker "<<np<<" attempted "<<nexatt[np]<<" exchanges with "<<nexsuc[np]<<" successes. Fraction: "<<fraction<<endl;
		logfile<<"Walker "<<np<<" attempted "<<nexatt[np]<<" exchanges with "<<nexsuc[np]<<" successes. Fraction: "<<fraction<<endl;
	}
//		coutfile << trial1.natoms << endl;
...
//	cout<<"nbonds "<<nbonds<<endl;
	...
	//cout<<"sumo "<<sumo<<endl;
	Q4a = sqrt((4.0*Pi/9.0)*sumo);
	}
	else{
		Q4a=1.0;
	}
	//Repeat procedure for second closest
	//find the atom second closest to COM
	long double rminb =1.0e10;
	for (unsigned int i = 0; i < c.natoms; ++i)
	{
		long double dist = c.ComDist(i);
			if (dist<rminb && dist>rmin){
				rminb=dist;
				mina=i;
			}
	}
