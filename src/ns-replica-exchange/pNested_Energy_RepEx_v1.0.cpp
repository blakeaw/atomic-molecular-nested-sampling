...
int main(int argc, char *argv[])
{
	...
	const unsigned int MCit = 100000;
	...
	string rundescript = "NS_n"+nds+"_b14";
...
logfile<<"Initial Energy: "<<Et1<<endl;
  ...
  //Perform markov moves on the configuration
    for(unsigned int n = 0; n<MCitpp; ++n){
		...
		if (tid<nprocs-1 && ran[1]<Pex){
			
			#pragma omp critical
			{
				#pragma omp flush(Tlocal)
				Config Swap(Tlocal[tid+1]);
				long double Etidp1=TotalEnergy(Swap);
				if (Etidp1<Enestloc[tid]){
					Tlocal[tid].Equate(Swap);
					Et1local[tid]=Etidp1;
				}
				#pragma omp flush(Tlocal, Et1local)
			}
		...
		//Coordinate change		
			...
			ny += movey;
			nz += movez;
			Tlocal[tid].WrapCoord(nx, ny, nz);

			//Check change in energy for pairwise interactions
			long double dEp=DeltaE(Tlocal[tid], Arandom, nx, ny, nz);
...
//			Tlocal.ReCenterZero();
			...
			//	long double dU = TestArea(Tlocal[tid], scale, Eelocal);
				...
				#pragma omp critical
				{
					#pragma omp flush(Ehist, Estat, Esqstat)
