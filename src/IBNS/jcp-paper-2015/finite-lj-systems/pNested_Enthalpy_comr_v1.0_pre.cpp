...
int main(int argc, char *argv[])
{
	...
	// Step interval to recenter the com to zero.
	const unsigned Centint = 5*natoms;
	//Boltzmann constant
//	const long double kb = 1.0;
	...
	const long double Vtop = 800.0;
	const long double Etop = 0.0;
	const long double Htop = Vtop*Press+Etop;
	//const long double Htop = Vtop*10.0;
	const long double Hlow = -10.0*(long double)(natoms);
	const long double binwidth = 1.0e-4;
	//const unsigned long nhbins = 1000000;
	const long double vfact = (4.0/3.0)*pi;
	long double Vexc = 1.0;
	const long double Vmax = (0.95*Vtop);
	...
	long double vstepguess = 5.0/Press;
	long double vstep = vstepguess;	


	//run descriptor
	string nds = itos(natoms);
	string rundescript = "NS_Ent_n"+nds+"_comr_pp_press_p_rep_rrreprr";
...
//------------------------------------------------------------
	...
	logfile<<"Constant Pressure Nested Sampling Simulation of LJ particles with a spherical center of mass boundary."<<endl;
	...
	logfile<<"dconstraint: "<<dconstraint<<" Htop: "<<Htop<<" Hlow: "<<Hlow<<" binwidth: "<<binwidth<<" Vmax: "<<Vmax<< " Vexc: "<<Vexc<<endl;	
	logfile<<"MCitpp: "<<MCitpp<<"ConfigsOutFreq: "<<ConfigsOutFreq<<endl;
	logfile<<"Vtop: "<<Vtop<<" Etop: "<<Etop<<endl;
...
logfile<<"starting radius of gyration: "<<trial1.RadGyr()<<endl;
cout<<"starting radius of gyration: "<<trial1.RadGyr()<<endl;
...
//	unsigned long long acceptcount = 0;
	...
	step=0.5*dclocal*pow(0.75, cfailcount);
  ...
  //Perform markov moves on the configuration
    for(unsigned int n = 0; n<MCitpp; ++n){
		...
		//Coordinate change		
			...
			Tlocal.CalcCom();
			bool disttag = Tlocal.CheckComDistAll(dclocal);
			//bool disttag = Tlocal.CheckComDist(Arandom, dclocal);
			//cout<<"dist : "<<dist<<" dclocal "<<dclocal<<" step "<<step<<endl;
			//if(dist>10.0){
			//	exit(0);
			//}
			if(!disttag){
				//dE += 10000.0;
				softsel = 0;
				//cout<<"breaking boundary!!"<<endl;
			}
			else{
				dE+=DeltaE(Tlocal, Arandom, origx, origy, origz);
				Et1local+=dE;
				//cout<<"dE: "<<dE<<" step "<<step<<endl;
			}
	...
	//		
	//	}
		if (ii==0 && Vcurr<Vtop && softsel && Ht1local<Htop){
		...
		//	}
			...
			else if(vdeftag==0 && softsel==1){
				++csuccessloc;			
			}	
		}
		else if(ii>0 && Ht1local < Hnest && softsel){
			//cout<<"NS "<<ii<<" Trial move was accepted! " << endl;
			tagg=0;
			Eelocal=Et1local;
			Helocal = Ht1local;
...
logfile<<" vfrac: "<<vafrac<<" vstep: "<<vstep<<endl;
