...
{
//Input File Name
string filename = "Hpoints_NS_Ent_n17_comr_p1.0_rep_0.dat";
string filename2 = "zObservables_NS_Ent_n17_comr_p1.0_rep_0.dat";
//Output File Name
string outfilename = "zNest_Cp_n17_p1.01_rep_0_test2.dat";
...
long double Vmax = 800.0;
long double K = pow(Vmax, N+1.0)/(N+1.0);
long double lK = log(K);
...
//	long double Obs7curr;
	...
	for(n=1;n<i-1;++n){
		...
		if(n==0){
			...
			Havgcurr=Obs1curr + Press*Obs2curr;
			if (Obs3curr>obs3max){
				obs3max=Obs3curr;
			}
			//cout<<"Hcurr: "<<Hcurr<<" Havg "<<Havgcurr<<endl;
...
//		cout<<"n: "<<n<<" obs1curr: "<<Obs1curr<<" obs2curr: "<<Obs2curr<<endl;
	...
	//	lw = log( pow(0.99, n-1) - pow(0.99, n) )-(Beta*Hcurr)+ N*log(Obs2curr);
...
//	long double ns = S/lwmax;
	// Internal Energy
	long double Znfid = Zest;
	unsigned long long nn = (unsigned long long)(N);
	long double Fid = (Beta*Press)/((long double)(factorial(nn))*pow(Beta, 3.0*N/2.0));
//	Zest*=Fid;
	//Zunscale/=Vlow;
...
//	long double G = -kb*Temp*log(Zunscale);
	...
	long double G = -kb*Temp*( log(Zest) + S + lK);
	long double Gid = kb*Temp*( log(intern/Obs2) );
	long double Gid2 = Gid = kb*Temp*( log(N*intern2) );
	//G correct
	long double sum=0.0;
	long double obs3c = 0.0;
	long double fn = (long double)(factorial(nn));
	for (unsigned int l = 0; l < nn-1; ++l)
	{
		
		long double fni = (long double)(factorial(nn-1-l));
		long double bpv = Beta*Press*Vmax;
		//cout<<"nn "<<nn<<" fn "<<fn<<" fni "<<fni<<" bpv "<<bpv<<endl;
//		if (l==0){
//			sum=log((fn*pow(bpv, nn-l)*exp(-bpv))/fni);
//		}
//		else{

//			sum*= log((fn*pow(bpv, nn-l)*exp(-bpv))/fni);
//		}
		sum+=(fn*pow(bpv, nn-l)*exp(-bpv))/fni;
		obs3c+=((fn*pow(bpv, nn-l)*exp(-bpv))/fni)*obs3max;
	}
	sum/=pow(Beta*Press, nn+1);
	long double Gc = -kb*Temp*( log(Zest*exp(S)*exp(lK) + sum));
	obs3c+=Obs3;
	obs3c/=(Zest+sum);
	long double Gi = -kb*Temp*log(fn/pow(Beta*Press, nn+1));
	//long double Gc = G*log(sum);
	//cout<<" G: "<<G<<" sum "<<sum<<" Gc "<<Gc<<endl;
...
//	cout<<"To file Obs1: "<<Obs1<<" Obs2: "<<Obs2<<" Zest: "<<Zest<<endl;	
	...
	//Output to file
	outfile<<std::scientific << setiosflags(ios::fixed)<< setprecision(15)<<Temp<<" "<<Zest<<" "<<H<<" "<<C<<" "<<Hid<<" "<<Cid<< " "<<Obs3<<" "<<G<<" "<<mu<<" "<<cvn<<" "<<Gc/N<<" "<<obs3c<<" "<<Entropy<<" "<<Gi/N<<" "<<Gid<<" "<<Gid2<<endl;
}
