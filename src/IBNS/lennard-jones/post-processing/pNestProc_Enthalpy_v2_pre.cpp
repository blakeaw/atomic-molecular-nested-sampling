...
long double tex;
int i = 0;
cout<<" reading in nested enthalpies"<<endl;
// Reads the file line by line until end 
while (! infile.eof()){
	infile >> tex;
	Hn[i] = tex;
	//cout<<" i: "<<i<<" tex: "<<tex<<endl;
	++i;
	if(i==200000-1){
		break;
	}
...
// Reads the file line by line until end 
cout<<"reading in observables"<<endl;
while (! infile2.eof()){
...
cout<<"beginning temperature loop"<<endl;
...
//	long double G = -kb*Temp*log(Zunscale);
	...
	Entropy+=(N+1.0)*log(800.0)+log(N+1.0)-(N+1.0)*log(Press);
	//Entropy -= (3.0*N/2.0)*log(Beta)+log(Beta);
	//Entropy*=-1.0;
...
cout<<"complete"<<endl;
cout<<"outfile is: "<<outfilename.c_str()<<endl;
outfile.close();
