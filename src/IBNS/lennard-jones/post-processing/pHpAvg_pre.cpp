...
cout << "# Averaging the Hpoint values..." << endl;
//start i at 1 to throw out first value which is incorrect when using Vmax
for(unsigned i=1;i<numpts;++i){
	for(unsigned j=0;j<numsets;++j){
		Cvstat.Push(log(Cv[i][j]));
	}
	long double stderror = Cvstat.StandDev() / rN;
	outfile << i << " " << Cvstat.Mean() << " " << stderror<< " "<<Cvstat.StandDev() << endl;
	Cvstat.Reset();
