/*
Program to read in temperature vs Obs values from a file
and average the Obs values
...
are any comments in the Obs outputs they need to be removed before/during
catting.
...
const unsigned numsets =n_sets_n;
//Number of data points in each set
const unsigned numpts = 4000;
//Number of columns in the file/set
const unsigned numcol = 13;
//Column number that temp is stored. Starts at 1
const unsigned tcol = 1;
//Column number that Obs is stored. Starts at 1
const unsigned ccol = o_col_o;
//Input file
string infilename = "i_file_i" ;
//Output file
string outfilename = "aNest_na_atoms_a_ttagt_pp_press_p_otago.dat";
...
long double N = (long double)(numsets);
long double rN = sqrt(N);
...
for(unsigned i =0;i<numsets;++i){
	for(unsigned k=0;k<numpts;++k){
		...
		for(unsigned j=0;j<numcol;++j){
			...
			if(j == ccol-1){
			//	cout << "Observable value " << input << " read in!" << endl;
				Cv[k][i]=input;
			}
		}
	}
...
cout << "# Averaging the Obs values..." << endl;
for(unsigned i=0;i<numpts;++i){
	for(unsigned j=0;j<numsets;++j){
		Cvstat.Push(Cv[i][j]);
	}
	long double stderror = Cvstat.StandDev() / rN;
	outfile << Temp[i] << " " << Cvstat.Mean() << " " << stderror<< " "<<Cvstat.StandDev() << endl;
	Cvstat.Reset();

}
outfile.close();
cout << "Process Complete. Check the output file: " << outfilename.c_str() << endl;
cout << " " << endl;
