...
long double PairEnergyFPNSI(const Atom& one, const Atom& two, const long double& boxd);
long double PairEnergyOFPNSI(const Atom& one, const long double& ox, const long double& oy, const long double& oz, const long double& boxd);
long double PairEnergyFPSI(const long double& boxd);
...
//	long double dconstraint = 6.60;
	...
	const long double Hlow = -50.0*(long double)(natoms);
  ...
  //Perform markov moves on the configuration
    for(unsigned int n = 0; n<MCitpp; ++n){
		...
		//Coordinate change		
			...
			dE+=DeltaEPeriodic(Tlocal, Arandom, origx, origy, origz);
			//Tlocal.CalcCom();
		...
		if(tagg == 1){
			...
			//	cout << "He*He: " << He*He << endl;	
				#pragma omp critical
				{
					...
					long double rho = (long double)(natoms)/Velocal;
					Rgstat.Push(rho);
					Hstat.Push(Helocal);
					Vstat.Push(Velocal);
				}
	...
	long double box = c.boxx;
		for(unsigned int pp = 0;pp<natoms-1;++pp){
			...
			for(unsigned int uu = pp+1;uu<natoms;++uu){
				...
				Etot += PairEnergyFPNSI(c.atom[pp], c.atom[uu], box);
				
			}
		}
	//Particles own periodic images
	Etot+=PairEnergyFPSI(box)*(long double)(natoms);	
//	Etot *= 60.0;
	return(Etot);
}
...
//	double rc = 3.0*sigma;
	...
	dz = one.z - two.z;
	potent = 0.0;
	int pmax = (int)(2.0*rc/box);
	if (pmax==0){
		
		//Box is Large- Use Minimum image -- coordinates must be pre-wrapped
		if(abs(dx)> box/2.0){
			dx = box - abs(one.x) - abs(two.x);
		}
		if(abs(dy)>box/2.0){
			dy = box - abs(one.y) - abs(two.y);
		}
		if(abs(dz)>box/2.0){
			dz = box - abs(one.z) - abs(two.z);
		}
		...
		//Box is small use fully periodic
		long double dxn, dyn, dzn;
		//x
		for (int i = -pmax; i <=pmax; ++i)
	{
			//y
			for (int j = -pmax; j <=pmax; ++j)
			{
			
				//z
				for (int k = -pmax; k <=pmax; ++k)
				{

					dxn=dx+(long double)(i)*box;
					dyn=dy+(long double)(j)*box;
					dzn=dz+(long double)(k)*box;
					rs=(pow(dxn, 2))+(pow(dyn, 2))+(pow(dzn, 2));
					if (rs<rcs){
						long double pot;
						if (i!=0 && j!=0 && k!=0){
							pot = 8.0*eps*((pow(sigma/rs, 6))-(pow(sigma/rs, 3))) - 8.0*eps*( pow(sigma/rcs, 6)-pow(sigma/rcs, 3) );
							potent+= pot;
						}
						else{
...
//	rs= (pow(dx, 2))+(pow(dy, 2))+(pow(dz, 2));
	...
	if (pmax==0){
		...
		//Box is Large- Use Minimum image -- coordinates must be pre-wrapped
		if(abs(dx)> box/2.0){
			dx = box - abs(one.x) - abs(ox);
		}
		if(abs(dy)>box/2.0){
			dy = box - abs(one.y) - abs(oy);
		}
		if(abs(dz)>box/2.0){
			dz = box - abs(one.z) - abs(oz);
		}
...
//	long double boxz=box;
	...
	for (int i = -pmax; i <=pmax; ++i)
	{
		...
		for (int j = -pmax; j <=pmax; ++j)
		{
			...
			{
				if (i!=0 && j!=0 && k!=0){
					dxn=(long double)(i)*box;
					dyn=(long double)(j)*box;
					dzn=(long double)(k)*box;
					rs=(pow(dxn, 2))+(pow(dyn, 2))+(pow(dzn, 2));
				if (rs<rcs){
					long double pot = 4.0*eps*((pow(sigma/rs, 6))-(pow(sigma/rs, 3))) - 4.0*eps*( pow(sigma/rcs, 6)-pow(sigma/rcs, 3) );
					potent+= pot;
				//	cout<<"FPSI rs: "<<rs<<" pot: "<<pot<<" potent: "<<potent<<endl;
				//	cout<<"i: "<<i<<" j: "<<j<<" k: "<<k<<endl;
				//	cout<<endl;
				}
				}
...
long double DeltaEPeriodic(const Config& c, const unsigned& Aindex, const long double& ox, const long double& oy, const long double& oz){
	...
	{	
		if(i!=Aindex){
			Etot += PairEnergyFPNSI(c.atom[Aindex], c.atom[i], box);
			Etoto += PairEnergyOFPNSI(c.atom[i], ox, oy, oz, box);
