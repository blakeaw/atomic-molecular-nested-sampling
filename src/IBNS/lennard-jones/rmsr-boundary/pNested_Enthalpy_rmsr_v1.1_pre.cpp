...
//	const long double Htop = Press*Vmax;
 ...
 //	long double Vexc = 0.5*(4.0/3.0)*pi*(long double)(natoms);
	...
	const long double rmax = pow(Vmax / vfact, -3.0);
	//run descriptor
	string nds = itos(natoms);
	string rundescript = "NS_Ent_n"+nds+"_rmsr_pp_press_p_rep_rrreprr";
...
//------------------------------------------------------------
	...
	logfile<<"rmax: "<<rmax<<" Htop: "<<Htop<<" Hlow: "<<Hlow<<" Vmax: "<<Vmax<< " Vexc: "<<Vexc<<endl;	
...
//	unsigned long long acceptcount = 0;
	...
	Vinitial = vfact*pow(rlocal, 3.0);
	step=bsfac*rlocal*pow(0.85, cfailcount);
