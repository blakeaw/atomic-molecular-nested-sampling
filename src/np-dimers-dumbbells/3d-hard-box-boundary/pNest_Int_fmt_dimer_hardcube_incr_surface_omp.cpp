...
#include "/net/uu/nm/cm/bxw109120/Dimers/NestedSampling/src/class/HistogramNS.h"
...
int main(int argc, char *argv[])
{
	...
	// the number of dimers to have in the system
	...
	const unsigned long int Iterations = 2;
	//Number of Markov style iterations to adjust configuration
	const unsigned long long MCit = 100000;
	...
	const unsigned short numreplicates = 4;
	...
	const unsigned long long nbins = 6000000;
	...
	//Output Files
	string rundescript = "cube_d2_test_omp";
	//Name of logfile
...
//----End of Parameter setting-----
   ...
   //Initialize output file streams
	...
	cout << "System running with " << ndimers << " dimers and has a total of " << natoms << " atoms" << endl;
...
//	Ehist.ResetObs();
	...
	for (unsigned long long int j = 0; j < MCit; ++j){
		...
		origx = confn.dimer[Dr].atom[Ar].x;
		origy = confn.dimer[Dr].atom[Ar].y;
		origz = confn.dimer[Dr].atom[Ar].z;
		confn.dimer[Dr].CalcCom();
		long double ocomx, ocomy, ocomz;
		ocomx = confn.dimer[Dr].COM[0];
		ocomy = confn.dimer[Dr].COM[1];
		ocomz = confn.dimer[Dr].COM[2];
		...
		confn.dimer[Dr].atom[Ar].x += movex;
		confn.dimer[Dr].atom[Ar].y += movey;
		confn.dimer[Dr].atom[Ar].z += movez;
		...
		else{confn.dimer[Dr].CalcCom();
			Ar2=0;
		}
		//Store original coordinates
		long double origx2, origy2, origz2, origr2;
		origx2 = confn.dimer[Dr].atom[Ar2].x;
		origy2 = confn.dimer[Dr].atom[Ar2].y;
		origz2 = confn.dimer[Dr].atom[Ar2].z;
		...
		if(ranprob<=0.50){
			...
			u = dsfmt_genrand_close_open(&dsfmt);
			v = dsfmt_genrand_close_open(&dsfmt);
			confn.dimer[Dr].atom[Ar2].x += movex;
			confn.dimer[Dr].atom[Ar2].y += movey;
			confn.dimer[Dr].atom[Ar2].z += movez;
			RandomRotation(u, v, stepa, confn.dimer[Dr], Ar);
		}
		else{
			confn.dimer[Dr].atom[Ar2].x += movex;
			confn.dimer[Dr].atom[Ar2].y += movey;
		...
		long double nz1, nz2;
		nz1=confn.dimer[Dr].atom[Ar].z;
		nz2=confn.dimer[Dr].atom[Ar2].z;
		long double r1, r2;
		r1 = confn.dimer[Dr].atom[Ar].rad;
		r2 = confn.dimer[Dr].atom[Ar2].rad;
		// Calculate the difference in total potential energy
		//cout << "Emedian: " << Emedian << endl;
		confn.dimer[Dr].CalcCom();
		long double ncomx, ncomy, ncomz;
		ncomx = confn.dimer[Dr].COM[0];
		ncomy = confn.dimer[Dr].COM[1];
		ncomz = confn.dimer[Dr].COM[2];
	...
	//	cout << "Dist is " << dist << endl;
		bool tagg = 1;
		bool dccheck = confn.BoxCheck(Dr);
		...
		bool hardsuccess = 0;
 		if(wallcheck == 0 && dccheck==0){
			...
			long double dEpp = DeltaE(confn, Dr, Ar, origx, origy, origz, origx2, origy2, origz2);
	
			long double dEww = dEWallZ(confn.dimer[Dr], origz, r1, origz2, r2, WallZ);
	...
	//	cout << "dE is now " << dE << " and Et1 plus dE is " << Et1 << endl;
	//	cout << endl;
		bool softsuccess = 0;
		...
		if(tagg == 1){
			...
			confn.dimer[Dr].atom[Ar].x = origx;
			confn.dimer[Dr].atom[Ar].y = origy;
			confn.dimer[Dr].atom[Ar].z = origz;
			confn.dimer[Dr].atom[Ar2].x = origx2;
			confn.dimer[Dr].atom[Ar2].y = origy2;
			confn.dimer[Dr].atom[Ar2].z = origz2;
			if(hardsuccess == 1){
				Et1=Et1s;
			//	Et1 = TotalPairEnergy(confn)+TotalWallEnergyZ(confn);
			}
	...
	//	cout << " Et1: " << Et1 << endl;
		...
		if(j>Eqit && j%(collectstep)==0){
			...
			//	long double xdedx = Xe*(abs(dEe)/dXe);
				...
				long double Rg, dynVol, dynDens, Zor, Dor, Zdist;			
				#pragma omp parallel sections num_threads(2)
				{	
					#pragma omp section
					{		
					 Rg = RadGyr(confn);
				 	 dynVol = (4.0/3.0)*pi*(Rg*Rg*Rg);	
					 dynDens = ((long double)(confn.ndimers)*2.0)/dynVol;
					 Zdist = AverageZwallDist(confn);
					 Estat.Push(Ee);
					 Esqstat.Push(Ee*Ee);
					 Rgstat.Push(Rg);
					 Zdstat.Push(Zdist);
					 ...
					 Ehist.PushObs(Ee*Ee, 1, Ee);
				 	 Ehist.PushObs(Rg, 2, Ee);
					 Ehist.PushObs(dynDens, 3, Ee);
					 Ehist.PushObs(Zdist, 6, Ee);
					}
					#pragma omp section
					{
				 	 
					 Zor = AverageZorientation(confn);
					 Dor= NPvecProd(confn);
					 Dstat.Push(dynDens);
					 Zdensity(confn, Gofz);
				   	 Zorstat.Push(Zor);
			    	 Dorstat.Push(Dor);
					 Ehist.PushObs(Zor, 4, Ee);
				     Ehist.PushObs(Dor, 5, Ee);
					}
				}
		...
		if(j>Eqit && j%ConfigsOutFreq ==0){
			...
			for(unsigned long long hg = 0; hg<natoms; hg++){
				...
				coordsfile << setiosflags(ios::fixed) << setprecision(15) << confn.dimer[Di].atom[Ai].type.c_str() << " " << confn.dimer[Di].atom[Ai].x << " " << confn.dimer[Di].atom[Ai].y << " " << confn.dimer[Di].atom[Ai].z << endl; 
			}
		}
...
long double PairSChargeShell(const Atom& one, const Atom& two, long double r, long double kappa){
	...
	long double potent;
	long double R1, R2, Ae1, Ae2;
	bool ctag1, ctag2, Ae;
//	long double k4 = pow(kappa, 4);
	ctag1 = one.ctag, ctag2 = two.ctag;
