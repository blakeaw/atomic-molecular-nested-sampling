...
int main(int argc, char *argv[])
{
	...
	const long double kappa = 0.3000;
	...
	//Output Files
	string rundescript = "cube_d4_test_testmb";
	//Name of logfile
	...
	//	cout << " Et1: " << Et1 << endl;
		...
		if(j>Eqit && j%(collectstep)==0){
			...
			//	long double xdedx = Xe*(abs(dEe)/dXe);
				...
				long double Rg = RadGyr(confn);
				long double dynVol = (4.0/3.0)*pi*(Rg*Rg*Rg);
				long double dynDens = ((long double)(confn.ndimers)*2.0)/dynVol;
				long double Zor = AverageZorientation(confn);
				long double Dor= NPvecProd(confn);
				long double Zdist = AverageZwallDist(confn);
