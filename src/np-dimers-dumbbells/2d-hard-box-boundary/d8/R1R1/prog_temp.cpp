...
void quickSort(long double* arr, int left, int right, unsigned long indexsort[]);
...
/*Name of file -- without file type i.e. 
  coord.xyz -> coord    */ 
string ifname = "ConfigsDimer_int_cube_d8_test_15_rep2";
...
//	cout<<"cind "<<cind<<" sel "<<sel<<endl;
	...
	if (i==0){
		//long double Emin = Energies[Eindexsort[0]];
		//long double Rs = 
...
void quickSort(long double* arr, int left, int right, unsigned long indexsort[]) {
      ...
      int i = left, j = right;
	//cout<<"left: "<<left<<" i "<<i<<" right "<<right<<" j "<<j<<endl;
     long double tmp;
	  unsigned long temp;
...
void POVRayOut(string* mat, const unsigned long& ccurr, const unsigned long Eindsort[], const unsigned& na, const unsigned& nb, const unsigned long long& cls, string inname, const long double& bz){
	...
	long double floor=0.0;
	long double top=bz/2.0;	
