...
{
//Input File Name
string filename = "Epoints_int_cube_dd_dim_d_test_itagi_reprrtagrr.dat";
string filename2 = "zNSobs_int_cube_dd_dim_d_test_itagi_reprrtagrr.dat";
//Output File Name
string outfilename = "zCv_int_cube_dd_dim_d_test_itagi_reprrtagrr.dat";
string outfilename2 = "zNestOut_int_cube_dd_dim_d_test_itagi_reprrtagrr.dat";
long double kb = 1.0;
long double Tstart = 0.0001;
long double Tfinish = 4.0;
unsigned long Tloop = 2000;
long double Beta;
long double Temp;

unsigned int ndimers = d_dim_d ;
...
// Reads the file line by line until end 
while (! infile2.eof()){
	infile2 >> junk >> tex1 >> tex2 >> tex3 >> tex4 >> tex5 >> tex6 >> tex7 >> tex8 >> tex9 >> tex10;
	//cout << tex1 << " "<<tex2<<" "<<tex3<<" "<<tex4<<" "<<tex5<<endl;
	Obs[kuku][0] = tex1; //Currently: Median Energy
	Obs[kuku][1] = tex2; //Currently: Mean Energy
	Obs[kuku][2] = tex3; //Currently: Mean Energy^2
	Obs[kuku][3] = tex4; //Currently: Radius of Gyration
	Obs[kuku][4] = tex5; //Currently: Gyration Density
	Obs[kuku][5] = tex6; //Currently: NP-NP Orientation Param
	Obs[kuku][6] = tex7; //Currently: HCP Packing Param
...
//	cout<<"Obs9b: "<<Obs9<<endl;
	long double Cvb = Obs3 - (Obs2*Obs2);
	Cvb *= (Beta/Temp);
	long double Cod = C/(long double)(ndimers);
	long double Uod = U/(long double)(ndimers);
	long double G = -kb*Temp*( log(Zest) + S);
	long double Ent = (U-G)/Temp;
//outfile2 << std::scientific << setiosflags(ios::fixed) << setprecision(15) << Temp << " " << Obs1 << " " << Obs2 << " " << Cvb <<  endl; 	
	...
	outfile << std::scientific << setiosflags(ios::fixed) << setprecision(15);
	 outfile << Temp << " " << Zest << " " << U << " " << C <<" "<<Uod<<" "<<Cod<< endl;

	outfile2 << std::scientific << setiosflags(ios::fixed) << setprecision(15);
	outfile2 << Temp << " " << Obs1 << " " << Obs2 << " " << Cvb << " " << Ent;
