...
long double kstart= 0.0001;
long double kstop = 1.0;
unsigned kloop = 25;
...
Rstart = R1+R2+0.5;
Rend = 15.0;
long double dk = (kstop-kstart)/(long double)(kloop);
//cout<<"Ae: "<<Ae<<endl;
ofstream outfile2("KappaDepend3d.kre");
for (unsigned int j = 0; j <= kloop; ++j)
{
	
kappa=kstart + dk*(long double)(j);
