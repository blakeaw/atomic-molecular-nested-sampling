...
logfile << " " << endl;
	...
	logfile<<"Constant Volume Nested Sampling of lennard jones system with continuum sphere fixed at origin"<<endl;
...
//			Tlocal.WrapCoordinates();
			...
			bool csflag = 0;
			long double dEp=0.0;
			long double dcs = CS.Dist(Tlocal.atom[Arandom]);
			//cout<<"dcs "<<dcs<<endl;
			if (dcs>srad){
				dEp=DeltaE(Tlocal, Arandom, origx, origy, origz)+CS.DeltaE(Tlocal.atom[Arandom], origx, origy, origz);
				csflag=1;
				//cout
			}
		...
		if(Et1local < Enest && Et1local>Elow && csflag){
...
//			Tlocal.ReCenterZero();
		...
		if(tagg == 1){
			...
			//	long double LD = LocalDensity(Tlocal);
				...
				long double Es = CS.TotalPotential(Tlocal);
				#pragma omp critical
				{
					Ehist.Push(Eelocal);
					Estat.Push(Eelocal);
