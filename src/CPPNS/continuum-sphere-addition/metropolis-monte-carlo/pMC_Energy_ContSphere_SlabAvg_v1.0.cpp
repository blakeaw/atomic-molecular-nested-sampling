...
long double SlabAverageRadialBulk(long double Ri, long double Rf, const Config& c);
long double SlabAverageRadialInterface(long double Ri, long double Rf, const Config& c, ContinuumSphere& s, long double& N);
...
int main(int argc, char *argv[])
{
	...
	//Temp bound
	long double Tstart = 1.0;
	long double Tend = 1.2;
	long double Tloop = 1;
	//Total number of MC style trial moves under each NS iteration
	const unsigned int MCit = 6000000;
	...
	const unsigned Eqit = 8000*natoms;
...
//	long double Binwidth = 1.0e-4;
	...
	string rundescript = "MC_SA_n"+nds+"_a0";
...
//			}
		...
		if(tagg == 1){
			...
			//	long double LD = LocalDensity(Tlocal);
				long double Eab = SlabAverageRadialBulk(2.0, 12.0, Tlocal);
				long double Ni;
				long double Eai = SlabAverageRadialInterface(2.0, 12.0, Tlocal, CS, Ni);
				long double dUa = Eai - Eab;
				//long double dUa = Eab - Eai;
				long double dU = dUa*Ni;
				long double edU = exp(-Beta*dU);
				long double dUdR = CS.TotalPotentialDerivative(Tlocal);
			//	cout<<"dU "<<dU<<" dUa "<<dUa<<" Eab "<<Eab<<" Eai "<<Eai<<" Ni "<<Ni <<" edU "<<edU<<endl;
				#pragma omp critical
				{
...
cout<<" Average Energy is "<<Eavg<<" Average Energy dUdR: "<<Esqavg<<endl;
logfile<<"Average Energy is "<<Eavg<<" and Average dUdR: "<<Esqavg<<endl;
cout<<"Average Ni: "<<LDavg<<endl;
logfile<<"Average Ni: "<<LDavg<<endl;
cout<<"Average e^dU: "<<dUavg<<endl;
logfile<<"Average e^dU: "<<dUavg<<endl;
cout<<"Average dU: "<<dUsavg<<endl;
logfile<<"Average dU: "<<dUsavg<<endl;
cout<<"Average Eab: "<<dUcavg<<endl;
logfile<<"Average Eab: "<<dUcavg<<endl;
cout<<"Average Eai: "<<Rgavg<<endl;
logfile<<"Average Eai: "<<Rgavg<<endl;
...
//	long double gamma = dUavg+Vterm;
//	long double gamma = -Temp*log(dUavg)/sai;
	long double gdv = -Temp*(101.0*log(Vti) - 100.0*log(Gvol) - log(Vi)); 
	long double gamma = (dUsavg)/sai;
	long double gam2 = (dUsavg+gdv)/sai;
	long double gam3 = (dUsavg-gdv)/sai;
	long double gam4 = (-Temp*log(dUavg)-gdv)/sai;
	long double gam5 = (-Temp*log(dUavg)+gdv)/sai;
	//gamma/=dA;
	cout<<"Temp: "<<Temp<<" gamma "<<gamma<<" dA "<<sai<<"  dUavg "<<dUavg<<" gdv "<<gdv<<" gam2 "<<gam2<<" gam3 "<<gam3<<" gam4 "<<gam4<<" gam5 "<<gam5<<endl;
	houtfile<<Temp<<" "<<gamma<<" "<<gam2<<" "<<gam3<<" "<<gam4<<" "<<gam5<<endl;
	long double dAdR = 8.0*pi*CS.R;
	long double gamgam = Esqavg/dAdR;
	cout<<"gamgam "<<gamgam<<" dUdR "<<Esqavg<<" dAdR "<<dAdR<<endl;
//dUcstat.Reset();
//Rgstat.Reset();
//if (Elconf<Enest){
//	trial1.Equate(Lconf);
...
long double SlabAverageRadialBulk(long double Ri, long double Rf, const Config& c){
	...
	long double by = c.boxy;
	long double bz = c.boxz;
	long double Etot = 0.0;
	long double Ntot = 0.0;
	for (unsigned int i = 0; i < c.natoms; ++i)
	{
		long double x, y, z;
		x=c.atom[i].x;
		...
		if (r>Ri && r<Rf){
			...
			{
				if (j!=i){
					Etot+=PairEnergy(c.atom[i], c.atom[j], bx, by, bz);
				}
			}
		}
	}
	long double Eavg = Etot/Ntot;
	return Eavg;
}
