...
//	boxz/=(sca*sca);
	...
	string rundescript = "NS_CT_n"+nds+"_d0";
...
//		}
	...
	Econf = CS.TotalPotential(trial1);
...
long double Egmin = CS.TotalPotential(trial1);
...
//			Tlocal.WrapCoordinates();
		...
		//	long double Enew = TotalEnergy(Tlocal);
			...
			long double Prat = ConfProb(dEp, Beta);
	...
	//	Et1local+=dE;
		...
		if(Et1local < Enest && Et1local>Elow && csflag && Prand<Prat){
