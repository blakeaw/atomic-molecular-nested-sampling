...
{
	infile>>junk>>x>>y>>z>>junk>>junk>>junk;
	//cout<<"x "<<x<<endl;
	trial1.atom[i].x=x;
	trial1.atom[i].y=y;
	trial1.atom[i].z=z;
...
cout<<"Making single atom perturbation to check delta e function..."<<endl;

unsigned int Arandom = (unsigned int)(dsfmt_genrand_close_open(&dsfmt)*natoms);
//Coordinate change		

			long double origx, origy, origz;
			origx = trial1.atom[Arandom].x;
			origy = trial1.atom[Arandom].y;
			origz = trial1.atom[Arandom].z;
			...
			trial1.atom[Arandom].x += movex;
			trial1.atom[Arandom].y += movey;
			trial1.atom[Arandom].z += movez;
			trial1.WrapAtom(Arandom);
			//Check change in energy for pairwise interactions
			long double dEp=DeltaE(trial1, Arandom, origx, origy, origz);
			long double EpdE = Egmin+dEp;
			long double Enew = TotalEnergy(trial1);
			cout<<"dEp "<<dEp<<" EpdE: "<<EpdE<<" Enew: "<<Enew<<endl;
