...
long double TotalEnergyTwo(const Config& c1, const Config& c2, long double sigma);
long double PairEnergy(const Atom& one, const Atom& two, const long double& boxx, const long double& boxy, const long double& boxz, long double sigma);
long double DeltaEtwo(const Config& c, const unsigned& Aindex, const long double& ox, const long double& oy, const long double& oz, const Config& s, long double sigma);
long double PairEnergyO(const Atom& one, const Atom& two, const long double& ox, const long double& oy, const long double& oz, long double bx, long double by, long double bz, long double sigma);
...
int main(int argc, char *argv[])
{
	...
	const unsigned int Nsamples = 60000;
...
//	boxz/=(sca*sca);
    ...
    long double sigma1 = 1.0;
    long double sigma2 = 2.0;
    long double sigma3 = 1.5;
	//Pi
...
//------------------------------------------------------------
    ...
    long double * Elist;
    Elist = new long double[Nsamples];
    long double * Elist2;
    Elist2 = new long double[Nsamples];
    long double * Elist2prev;
    Elist2prev = new long double[Nsamples];
	//Name of logfile
...
long double Egmin = ( TotalEnergyTwo(trial1a, trial1b, sigma2)-TotalEnergyTwo(trial1a, trial1b, sigma1) );
totalpeval += Nettwo;
...
dquickSort(Elist2, 0, Nsamples-1);
...
string eoname2 = ehistoutname+"_list2_"+"_it_"+itos(ii-1)+".dat";
Histogram Eohist;
Histogram Eohist2;
long double Eohistlow=Elist[0];
long double Eohisthigh=Elist[Nsamples-1];
long double Eohistlow2=Elist2[0];
long double Eohisthigh2=Elist2[Nsamples-1];
//if (Eohisthigh>100.0){
//    Eohisthigh=100.0;
//}
long double Eohistbinw = (Eohisthigh-Eohistlow)/500.0;
cout<<"Elow: "<<Eohistlow<<" Ehigh: "<<Eohisthigh<<" bw "<<Eohistbinw<<endl;
Eohist.Initialize(Eohistbinw, Eohistlow, Eohisthigh);
long double Eohistbinw2 = (Eohisthigh2-Eohistlow2)/500.0;
cout<<"Elow2: "<<Eohistlow2<<" Ehigh2: "<<Eohisthigh2<<" bw2 "<<Eohistbinw2<<endl;
Eohist2.Initialize(Eohistbinw2, Eohistlow2, Eohisthigh2);
for (unsigned int i = 0; i < Nsamples; ++i)
{
    //Eohist.PushWithWeight(Elist[i], exp(-Beta*Elist[i]);
    Eohist.PushOverTop(Elist[i]);
    Eohist2.PushOverTop(Elist2[i]);
}
Eohist.Normalize();
Eohist.DumpNormal(eoname);
Eohist.Reset();
Eohist2.Normalize();
Eohist2.DumpNormal(eoname2);
Eohist2.Reset();
if (ii>1){
    Histogram Eohist2prev;
    long double Eohistlow2prev=Elist2prev[0];
    long double Eohisthigh2prev=Elist2prev[Nsamples-1];
    //if (Eohisthigh>100.0){
    //    Eohisthigh=100.0;
    //}
    
    long double Eohistbinw2prev = (Eohisthigh2prev-Eohistlow2prev)/500.0;
   // cout<<"Elow2: "<<Eohistlow2<<" Ehigh2: "<<Eohisthigh2<<" bw2 "<<Eohistbinw2<<endl;
    Eohist2prev.Initialize(Eohistbinw2prev, Eohistlow2prev, Eohisthigh2prev);
    for (unsigned int i = 0; i < Nsamples; ++i)
    {
        //Eohist.PushWithWeight(Elist[i], exp(-Beta*Elist[i]);
     
        Eohist2prev.PushOverTop(Elist2prev[i]);
    }

    Eohist2prev.Normalize();
    //Eohist2prev.DumpNormal(eoname2);
    long double OverlapPrev = Eohist2prev.IntegralFractionAt(Elist2[Nsamples-1]);
    cout<<"Overlap with previous is : "<<OverlapPrev<<endl;
    Eohist2.Reset();
}

for (unsigned int i = 0; i < Nsamples; ++i)
{
    Elist2prev[i]=Elist2[i];
}
//Ehist.SmartReset();
//Hhist.Reset();
Estat.Reset();
...
//	omp_set_num_threads(nprocs);
    ...
    long double Es1 = TotalEnergyTwo(Tlocala, Tlocalb, sigma1);
    long double Es2 = TotalEnergyTwo(Tlocala, Tlocalb, sigma2);
  //Perform markov moves on the configuration
    while(nsamp<Nsamples){
        ...
        long double Es1b = Es1;
        long double Es2b = Es2;
		//Trial Move -----
        ...
        long double dEs1 = 0.0;
        long double dEs2 = 0.0;
...
//			Tlocal.WrapCoordinates();
			...
			if (pflag==0){
				dE = DeltaE(Tlocala, Arandom, origx, origy, origz);
                 dEs2=TotalEnergyTwo(Tlocala, Tlocalb, sigma2)-Es2b;
                dEs1=TotalEnergyTwo(Tlocala, Tlocalb, sigma1)-Es1b;
//                dEs2=DeltaEtwo(Tlocala, Arandom, origx, origy, origz, Tlocalb, sigma2);
//                dEs1=DeltaEtwo(Tlocala, Arandom, origx, origy, origz, Tlocalb, sigma1);
                dEtwo = dEs2 - dEs1;
               // dEtwo=(TotalEnergyTwo(Tlocala, Tlocalb, sigma2)-TotalEnergyTwo(Tlocala, Tlocalb, sigma1)) - Et1b;
				if (ii==0 && n<Eqit){
					totalpeval+=Ndea;
				}
				else{
                ...
                dEs2=TotalEnergyTwo(Tlocalb, Tlocala, sigma2)-Es2b;
                dEs1=TotalEnergyTwo(Tlocalb, Tlocala, sigma1)-Es1b;
//                dEs2=DeltaEtwo(Tlocalb, Arandom, origx, origy, origz, Tlocala, sigma2);
//                dEs1=DeltaEtwo(Tlocalb, Arandom, origx, origy, origz, Tlocala, sigma1);
                dEtwo = dEs2 - dEs1;
		...
		//	long double Enew = TotalEnergy(Tlocal);
			...
			long double Prat = ConfProb(dE+dEs1, Beta);
			++ctriesloc;
		...
		//Add difference to current total potential energy
            ...
            Es1+=dEs1;
            Es2+=dEs2;
           // cout<<"Et1local: "<<Et1local<<" dEtwo: "<<dEtwo<<" Et1b "<<Et1b<< " dEs1 "<<dEs1<<" dEs2 "<<dEs2<<endl;
//			if (ii==0 && pflag==0){
//				Et1local=TotalEnergyTwo(Tlocala, Tlocalb, sigma2)-TotalEnergyTwo(Tlocala, Tlocalb, sigma1);
//			}
			//totalpeval+=Nettwo;
...
//				}
//			
//				cout<<"pflag "<<pflag<<" dE "<<dE<<" Ar "<<Arandom<<" Prand "<<Prand<<" Prat "<<Prat<<" Et1b "<<Et1b<<" Et1local "<<Et1local<<" Enest "<<Enest<<" Et1local full "<<TotalEnergyTwo(Tlocala, Tlocalb, sigma2)-TotalEnergyTwo(Tlocala, Tlocalb, sigma1)<<" dEs1: "<<dEs1<<" dEs2 "<<dEs2<<endl;
//    //<<" dEtwo "<<dEtwo<<" dEtwo recomp: "<<DeltaEtwo(Tlocala, Arandom, origx, origy, origz, Tlocalb)<<" dEtwodir "<<dEtwodir<<endl;
//				exit(0);
//			}
			//cout<<endl;
...
//				long double dU = TestArea(Tlocal, scale, Eelocal);
				...
				long double dU = 0.0;
                long double Es3 = ( TotalEnergyTwo(Tlocala, Tlocalb, sigma3)-TotalEnergyTwo(Tlocala, Tlocalb, sigma1) );
               // cout<<"Es3: "<<Es3<<" Eelocal: "<<Eelocal<<endl;
				#pragma omp critical
				{
					//Ehist.Push(Eelocal);
					Elist[nsamp]=Eelocal;
	...
	bx = c1.boxx, by = c1.boxy, bz = c1.boxz;
		for(unsigned int pp = 0;pp<na;++pp){
			...
			for(unsigned int uu = 0;uu<nb;++uu){
				...
				Etot += PairEnergy(c1.atom[pp], c2.atom[uu], bx, by, bz, sigma);
...
long double DeltaEtwo(const Config& c, const unsigned& Aindex, const long double& ox, const long double& oy, const long double& oz, const Config& s, long double sigma){
	...
	{
		//if(i!=Aindex){
			Etot += PairEnergy(s.atom[i], c.atom[Aindex], bx, by, bz, sigma);
			Etoto += PairEnergyO(s.atom[i], c.atom[Aindex], ox, oy, oz, bx, by, bz, sigma);
