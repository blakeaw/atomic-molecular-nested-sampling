...
int main(int argc, char *argv[])
{
	...
	const unsigned int MCit = 12000000;
	...
	const unsigned Cit = natoms;
	long double NSfrac = 0.5;
	//Box sizes
	long double boxx = 6.1;//*1.1;
	long double boxy = 6.1;//*1.1;
	long double boxz = 6.1;///(1.1*1.1);
...
//	boxz/=(sca*sca);
	...
	const long double Elow = -20.0;
	long double Binwidth = 1.0e-5;

		//run descriptor
	string nds = itos(natoms);
	string rundescript = "NS_n"+nds+"_h1";
	//coordinates to read in
	string infilename = "LJ51_GlobalMin.txt";
	ifstream infile;
...
if(ii==0){// randomly select an atom index	
			unsigned int Arandom; 
		

			//phase flag: 0 for a and 1 for b
			bool pflag;
			long double rpt = dsfmt_genrand_close_open(&dsfmt);
			if (rpt<0.10){
				pflag=1;
				Arandom= (unsigned int)(dsfmt_genrand_close_open(&dsfmt)*nb);
			}
			else{
				pflag=0;
...
//			}
		// randomly select an atom index	
			...
			if (pflag==0){
				...
				long double dE = 0.0;
				bool sflag=0;
				unsigned int Arandom2; 
			if (n%100==0){
				//swap
				sflag=1;
				
				if (pflag==0){
					Arandom2 = (unsigned int)(dsfmt_genrand_close_open(&dsfmt)*(natoms-na));
					Tlocala.atom[Arandom].Equate(Tlocalb.atom[Arandom2]);
					Tlocalb.atom[Arandom2].x=origx;
					Tlocalb.atom[Arandom2].y=origy;
					Tlocalb.atom[Arandom2].z=origz;
					
				}
				else if (pflag==1){
					Arandom2 = (unsigned int)(dsfmt_genrand_close_open(&dsfmt)*(natoms-nb));
					Tlocalb.atom[Arandom].Equate(Tlocala.atom[Arandom2]);
					Tlocala.atom[Arandom2].x=origx;
					Tlocala.atom[Arandom2].y=origy;
					Tlocala.atom[Arandom2].z=origz;
		...
		//Coordinate change		
			...
			long double rpc;
				...
				if (rpa<rpc){
					long double nnx, nny, nnz;
					nnx = dsfmt_genrand_close_open(&dsfmt)*boxx;
					nny = dsfmt_genrand_close_open(&dsfmt)*boxy;
					nnz = dsfmt_genrand_close_open(&dsfmt)*boxz;
					Tlocalb.atom[Arandom].x=nnx;
					Tlocalb.atom[Arandom].y=nny;
					Tlocalb.atom[Arandom].z=nnz;
				}
				else{
...
//			Tlocal.WrapCoordinates();
			...
			//	cout<<"dE "<<dE<<" sflag "<<sflag<<endl;
				if (sflag){
					long double ox, oy, oz;
					ox=Tlocala.atom[Arandom].x;
					oy=Tlocala.atom[Arandom].y;
					oz=Tlocala.atom[Arandom].z;
					dE+=DeltaE(Tlocalb, Arandom2, ox, oy, oz);
				}
			//	cout<<"dE "<<dE<<endl;
			}
			else if (pflag==1){
				...
				if (sflag){
					long double ox, oy, oz;
					ox=Tlocalb.atom[Arandom].x;
					oy=Tlocalb.atom[Arandom].y;
					oz=Tlocalb.atom[Arandom].z;
					dE+=DeltaE(Tlocala, Arandom2, ox, oy, oz);
				}
			}
		...
		if(Et1local < Enest && Et1local>Elow && Prat>Prand){
...
//			Tlocal.ReCenterZero();
		...
		if(tagg == 1){
				...
				if (pflag==0){
					if (sflag){
						Tlocalb.atom[Arandom2].Equate(Tlocala.atom[Arandom]);
					}
				...
				Tlocala.atom[Arandom].z = origz;
					...
					if (sflag){
					Tlocala.atom[Arandom2].Equate(Tlocalb.atom[Arandom]);
				}
			...
			if(n>Eqit && Et1local<Enest && pflag){
