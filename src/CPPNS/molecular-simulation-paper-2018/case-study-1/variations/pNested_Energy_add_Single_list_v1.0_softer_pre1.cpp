...
//------------------------------------------------------------
   ...
   // long double * Ereflist;
   // Ereflist = new long double[Nsamples];
	//Name of logfile
...
long double Egmin = TotalEnergyTwo(trial1a, trial1b);//-TotalEnergyTwo(trial1a, trial1b, sigma1);
totalpeval += Nettwo;
...
//	omp_set_num_threads(nprocs);
    ...
    long double Eref = TotalEnergy(Tlocala);
    cout<<"Intital Reference Potential: "<<Eref<<endl;	
...
//			Tlocal.WrapCoordinates();
			...
			if (pflag==0){
				...
				//dEtwo=DeltaEtwo(Tlocala, Arandom, origx, origy, origz, Tlocalb, sigma2)-DeltaEtwo(Tlocala, Arandom, origx, origy, origz, Tlocalb, sigma1);
...
//	eps = sqrt(one.eps*two.eps), sigma = (one.sig+two.sig)/2.0;
	...
	if (rs<rcs){
		potent = 4.0*eps*((pow(sigma/rs, 3))-(pow(sigma/rs, 2))) - 4.0*eps*((pow(sigma/rcs, 3))-(pow(sigma/rcs, 2)));
	}
	else{
		potent=0.0;
	}	
