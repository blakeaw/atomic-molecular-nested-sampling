...
int main(int argc, char *argv[])
{
	...
	const unsigned int Nsamples = 30000;
...
//Histogram Eohist;
//long double Eohistlow=Elist[0]-1.0;
//long double Eohisthigh=Elist[Nsamples-1]+1.0;
...
if (Nenest>0){
	...
	long double Aprev = -Temp*Cmax*log(Qmax)+Temp*Cmax*log(Wsmax);
      long double Ws = pow(NSfrac, Nenest-1) - pow(NSfrac, Nenest);
	long double Qn = (Ws)*exp(-Cmax*Enests[Nenest-1]/Temp);
  
	Qmax+= Qn;
    Wsmax+=Ws;
    long double Anew = -Temp*Cmax*log(Qmax)+Temp*Cmax*log(Wsmax);
