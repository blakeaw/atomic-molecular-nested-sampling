...
int main(int argc, char *argv[])
{
	...
	long double Wd = 16.0;
    long double kharm=0.001;
    // probability to move back two energy levels -- starts on iteration 2
    long double bprob = 0.750000;
...
logfile << " " << endl;
    ...
    logfile<<"Nsamples: "<<Nsamples<<endl;
	logfile<<"NS fraction : "<<NSfrac<<endl;
    logfile<<"Start One-sided harmonic bias parms;"<<endl;
	logfile<<"Initial kharm: "<<kharm<<endl;
    logfile<<" Two level energy step back probability (bprob): "<<bprob<<endl;
     logfile<<"End One-sided harmonic bias parms;"<<endl;
...
Frame BotFrame;
BotFrame.InitializeAtoms(natoms);
BotFrame.SetBox(boxx, boxy, boxz);
//Egmin*=0.1;
...
//    long double bprob = 0.25000;
//    long double deltaEm = Enest-Elist[Nelist-1];
...
//    //recompute the value of kharm 
    ...
    //long double deltaEm = Enest-Elist[Nelist-2];
     long double deltaEm = Elist[Nelist-2]-Elist[Nelist-3];
    kharm = -(2.0*log(bprob))/( Beta*deltaEm*deltaEm);
    //kharm=0.5;
    cout<<"kharm adjusted to :"<<kharm<<endl;
}
...
//if (ii>0){
    ...
    //cout<<"Enes
    long double Enests = (Enest+Enprev)*0.500;
	long double Qn = (pow(NSfrac, ii-1) - pow(NSfrac, ii))*exp(-alphamax*Enests/Temp);
    cout<<" Enests: "<<Enests<<" Qn: "<<Qn<<endl;
	Qmax+= Qn;
    cout<<" Qmax: "<<Qmax<<endl;
	long double qratio = Qn/Qmax;	
	cout<<"Qratio for alphamax="<<alphamax<<" is : "<<qratio<<endl;
    long double Ediff = abs(Enest-Enprev);
	cout<<"Ediff: "<<Ediff<<endl;
	if (qratio<sratio){
		cout<<"Qratio "<<qratio<<" less than sratio "<<sratio<<endl;
		cout<<"Stopping criteria for alphamax "<<alphamax<<" is satisfied.."<<endl;
		break;
	}
//}
Enprev=Enest;
      ...
      frame[0].t = 0.0; // sets initial time to zero!
       ...
       // correct(frame[0]);
        //if(ii==0){
            ...
            FIREmin(frame[0]);
            BotFrame.equate(frame[0]);
            long double botE = PotentialEnergy(BotFrame);
            cout<<"botE: "<<botE<<endl;
           // Elowprev=botE;
        } 
        else{

            frame[0].equate(BotFrame);
        }
...
void PairForceLJ(const AtomMD& one, const AtomMD& two, long double bx , long double by, long double bz, long double Fv[3]){
	...
	long double r = sqrt(rs);
	if (r<rc){
       long double Fpre = 4.0*eps*( -12.0/pow(r, 13) + 6.0/pow(r, 7) ); 
		Fx = Fpre*dx;
        Fy = Fpre*dy;
        Fz=Fpre*dz;
	}
