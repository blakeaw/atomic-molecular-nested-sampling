...
void AddBiasingForceES(const Frame& c, long double k, long double Uo);
...
//	boxz/=(sca*sca);
	...
	string rundescript = "NS_D_n"+nds+"_binaryLJSE_kappa1_t2_rep0";
...
long double Enest = Etop+400000.0;;
...
RunningStats Timer;
  ...
  //Initialize MD Copy Configs into an MD frame
      ...
      double timestep = 0.0005000; //timestep (femtosecond)
...
//        }
       // Et1local = TotalEnergyES(Tlocala)+TotalEnergyES(Tlocalb)+TotalEnergyTwoES(Tlocala, Tlocalb);
        long double Et1local1 = TotalEnergyES(frame[1]);
        if (ii>0){
           // long double bias;
            //if (Et1local>Enest){
//               bias = -kharm*(Et1local-Enest)/(3.0*(long double)(natoms));
     ...
     //   cout<<"frame potential energy via MC: "<<Eljtot<<endl;
       // exit(0)	;
		if (n%sout==0){
            ...
            printlog(frame[0], n);
             cout<<"Current Screened ES energy: "<<Et1local<<" Current nested value: "<<Enest<<endl;
		}
...
//				long double dU = TestArea(Tlocal, scale, Eelocal);
				...
				//{
					...
					//Ehist.Push(Et1local);
                    if (ii>0){
                        Ehist.PushToNormWithWeight(Et1local, exp(Et1local/Enest - 1.0));
                    }
                    else{
                        Ehist.PushToNormWithWeight(Et1local, 1.0);
                    }
					Estat.Push(Et1local);
		...
		for(unsigned int pp = 0;pp<natoms-1;++pp){
			...
			for(unsigned int uu = pp+1;uu<natoms;++uu){
               ...
               // long double pre = -exp(-Eij/Uo)/Uo;
                long double pre = -exp(1.0-(Eij/Uo))/Uo;
                long double Fbix,Fbiy,Fbiz,Fbjx,Fbjy,Fbjz;
	            Fbix = pre*Fi[0];
                Fbiy = pre*Fi[1];
                Fbiz = pre*Fi[2];
