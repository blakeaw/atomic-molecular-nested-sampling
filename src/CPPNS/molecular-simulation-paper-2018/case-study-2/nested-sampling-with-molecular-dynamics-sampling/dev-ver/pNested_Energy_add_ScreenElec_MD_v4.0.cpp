...
void FIREmin(Frame& f);
long double CalcP(Frame& f);
void ScaleVelocityFire(Frame& f, long double alpha);
void ZeroVelocityFire(Frame& f);
void pbc (long double &dx, long double &dy, long double &dz, double boxx, double boxy, double boxz);
long double exp7(long double x);
...
int main(int argc, char *argv[])
{
	...
	const unsigned int Nsamples = 4000;
	...
	const unsigned Cit = natoms/4;
	long double NSfrac = 0.60;
	//Box sizes
...
//	boxz/=(sca*sca);
	...
	//Enthalpy bounds
	const long double Etop = 1000000.0;
	const long double Elow = -1000.0;
	long double Binwidth = 1.0e-3;

		//run descriptor
	string nds = itos(natoms);
	string rundescript = "NS_D_n"+nds+"_binaryLJSE_kappa1_t1_rep0";
...
cout << "Nested sampling iterations is set to " << Iterations << " with MC iterations set to " << MCit << "\n";
cout<<"Etop: "<<Etop<<" Elow: "<<Elow<<"\n";	

logfile << " " << "\n";
	logfile << "Local time and date: " << asctime(timeinfo) << "\n";
	logfile<<"Constant Volume Nested Sampling of binary lennard jones system with charge. Charge is screened electrostatics with kappa=1. The cutoff for all interactions is rc=3.0"<<"\n";
	logfile<<" Diffusive sampling with weight: "<<Wd<<"\n";
	logfile << "Parameters: " << "\n";
	logfile<<"NS Iterations: "<<Iterations<<" Number of Trial Moves (MCit): "<<MCit<<"\n";
	logfile<<"NS fraction : "<<NSfrac<<"\n";
	logfile<<"TAM scaling factor: "<<scale<<"\n";
	logfile<<" na: "<<na<<" nb: "<<nb<<"\n";
	logfile<<"natoms: "<<natoms<<" nprocs: "<<nprocs<<" Eqit: "<<Eqit<<" Cit: "<<Cit<<"\n";
	logfile<<" boxx: "<<boxx<<" boxy: "<<boxy<<" boxz: "<<boxz<<"\n";
	logfile<<"bsfac: "<<bsfac<<" initial guess stepx: "<<stepx<<" stepy: "<<stepy<<" "<<stepz<<"\n";
	logfile<<" Etop: "<<Etop<<" Elow: "<<Elow<<"\n";	
	logfile<<"MCitpp: "<<MCitpp<<"ConfigsOutFreq: "<<ConfigsOutFreq<<"\n";
	...
	HistogramNS Ehist;
    //Histogram Ehist;
	Ehist.Initialize(Binwidth, Elow, 400.0);
...
//	RunningStats Rgstat;
	...
	logfile<<"number of NS bins: "<<Ehist.GetNumberOfBins()<<"\n";
	logfile<<"Potential Energy Histogram Params: "<<"\n";
	logfile<<"number of bins: "<<Eohist.GetNumberOfBins()<<" bindwidth: "<<Eohist.GetIncrement()<<" Elow: "<<Elow<<" Etop: "<<ldhigh<<"\n";
...
cout<<"Assigning Initial coordinates.."<<"\n";
logfile<<"Assigning Initial coordinates.."<<"\n";
...
//Eohist.Reset();
	...
	cout<<Econf<<" "<<Econf2<<" "<<Econf3<<" "<<Econf4<<"\n";
...
logfile<<"Initial Energy: "<<Et1<<endl;
	...
	// Config Tlocold(natoms);
      ...
      frame[0].t = 0.0; // sets initial time to zero!
        ...
        if(ii==0){
            ...
            cout<<"initial frame potential energy: "<<potentialEnergy<<endl;
            FIREmin(frame[0]);
        } 
    ...
    long double Et1local = TotalEnergyES(frame[0]);
     //Et1local*=0.1;
...
void correct (Frame &qqq){
	...
	for (int i = 0; i < qqq.natoms; i++) 
	{
	while (qqq.atom[i].x > qqq.boxx) {qqq.atom[i].x = qqq.atom[i].x - qqq.boxx;}
	while (qqq.atom[i].y > qqq.boxy) {qqq.atom[i].y = qqq.atom[i].y - qqq.boxy;}
	while (qqq.atom[i].z > qqq.boxz) {qqq.atom[i].z = qqq.atom[i].z - qqq.boxz;}
	while (qqq.atom[i].x < 0.0) {qqq.atom[i].x = qqq.atom[i].x + qqq.boxx;}
	while (qqq.atom[i].y < 0.0) {qqq.atom[i].y = qqq.atom[i].y + qqq.boxy;}
	while (qqq.atom[i].z < 0.0) {qqq.atom[i].z = qqq.atom[i].z + qqq.boxz;}	
     }
}
...
//        cout<<"r "<<r<<endl;
//        potent = (A/(r*kappa))*exp(-kappa*r) ;//- (A/(rc*kappa))*exp(-kappa*rc);
//        cout<<"potent: "<<potent<<endl;
//        potent = (A/(r*kappa))*exp7(-kappa*r);// - (A/(rc*kappa))*exp7(-kappa*rc);
//        cout<<"potent: "<<potent<<endl;
//		potent = (A/(r*kappa))*exp1(-kappa*r);// - (A/(rc*kappa))*exp1(-kappa*rc);
//        cout<<"potent: "<<potent<<endl;
//        exit(0);
	}
	else{
...
void FIREmin(Frame& f){
 ...
 // Initialization
	   Frame *frame;
	    ...
	    cout<<" beggining FIRE MD minimizer!"<<endl;
        cout<<" nsteps: "<<nsteps<<" initial timestep: "<<timestep<<" alphastart: "<<alphastart<<endl;
        cout<<" finc: "<<finc<<" fdec: "<<fdec<<" falpha: "<<falpha<<" Nmin: "<<Nmin<<endl;
        cout<<" dtmax: "<<dtmax<<endl;
		
        int natoms = f.natoms;
        frame[0].InitializeAtoms(natoms);
        frame[1].InitializeAtoms(natoms);
        frame[0].rCutOff=f.rCutOff;
        frame[1].rCutOff=f.rCutOff;
		//copy coordinates of input frame and set 
        // zero forces and velocities
        frame[0].equate(f);
        frame[1].equate(frame[0]);
		for (int i = 0; i < frame[0].natoms; ++i)
		{
//            long double x, y, z;
//            x= f.atom[i].x;
//            y = f.atom[i].y;
//            z = f.atom[i].z;
////			frame[0].atom[i].x=f.atom[i].x;
////			frame[0].atom[i].y=f.atom[i].y;
////			frame[0].atom[i].z=f.atom[i].z;
//            //cout<<"x "<<x<<" y "<<y <<" z "<<z<<endl;
//            frame[0].atom[i].x=x;
//            frame[0].atom[i].y=y;
//            frame[0].atom[i].z=z;
           // cout<<"frame0 x "<<frame[0].atom[i].x<<endl;
			frame[0].atom[i].v_x=0.0;
			frame[0].atom[i].v_y=0.0;
			frame[0].atom[i].v_z=0.0;
            ...
            frame[1].atom[i].v_x=0.0;
			frame[1].atom[i].v_y=0.0;
			frame[1].atom[i].v_z=0.0;
			frame[1].atom[i].f_x=0.0;
			frame[1].atom[i].f_y=0.0;
			frame[1].atom[i].f_z=0.0;
		}
    	//copy box size
		frame[0].boxx=f.boxx;
        frame[0].boxy=f.boxy;
 ...
 //  ** MD MAIN LOOP **
        for (int i = 0; i < nsteps; i++)
            ...
            printlog(frame[0], nsteps);
            f.equate(frame[0]);
            printlog(f, nsteps+1);
            cout<<"Exiting FIRE minimization!"<<endl;
            return;
}


//Fire minimize functions
long double CalcP(Frame& f){
