...
int main(int argc, char *argv[])
{
	...
	const unsigned int ncut = (unsigned int) ( Nsmps+1.0- NSfrac*(Nsmps+1.0) );
...
//	boxz/=(sca*sca);
	...
	string rundescript = "NS_rem_n"+nds+"_T2.0_ff_frac_f_rhox_dens_x_repx_rep_x";
...
logfile << " " << endl;
	...
	logfile<<"Constant Volume Nested Sampling of lennard jones system for Chemical Potential"<<endl;
    logfile<<" Includes particle swapping move followed by push back below current Nested Energy.."<<endl;
    logfile<<" .. suspending normal acceptance rules. Then a relaxation period to restablish 'equilibrium'"<<endl;
    ...
    logfile<<"nswap: "<<nswap<<" maxswaps: "<<maxswaps<<endl;
    logfile<<" Number of relaxation steps after swap (relaxstep): "<<relaxstep<<endl;
...
cout<<"Lowest Energy sampled: "<<-Elist[Nsamples-1]<<endl;
//exit(0);
if( ((ii-1)%100)==0){
string eoname = ehistoutname+"_it_"+itos(ii-1)+".dat";
Histogram Eohist;
long double Eohistlow=-Elist[Nsamples-1]-1.0;
long double Eohisthigh=-Elist[0]+1.0;
if (Eohisthigh>100.0){
    Eohisthigh=100.0;
}
long double Eohistbinw = (Eohisthigh-Eohistlow)/800.0;
...
if (Nenest>0){
    ...
    long double Aprev = -Beta*Cmax*log(Qmax)+Beta*Cmax*log(Wsmax);
      long double Ws = pow(NSfrac, Nenest-1) - pow(NSfrac, Nenest);
	long double Qn = (Ws)*exp(-Cmax*Enests[Nenest-1]/Temp);
  
	Qmax+= Qn;
    Wsmax+=Ws;
    long double Anew = -Beta*Cmax*log(Qmax)+Beta*Cmax*log(Wsmax);
    long double Adiff = abs(Anew-Aprev);
	long double qratio = Qn/Qmax;	
	cout<<"Qn "<<Qn<<" Qmax "<<Qmax<<" Qratio for Cmax="<<Cmax<<" is : "<<qratio<<endl;
	cout<<"Ediff: "<<Ediff<<endl;
    cout<<"Aprev "<<Aprev<<" Anew "<<Anew<<" Adiff "<<Adiff<<endl;
	if (qratio<sratio){
...
//energy difference stopping criterion
//if (Ediff<0.001){
//    cout<<"Energy difference stopping criterion met..."<<endl;
//	break;
//}
}
...
//		    
//		
////		    if (swflag && (Et1bprev>Enest && Et1local<Enest) ){
////		        n=0;
////                Eqit=relaxstep;
////                cout<<"Et1bprev: "<<Et1bprev<<" Et1local: "<<Et1local<<" Enest "<<Enest<<endl;
////		    }
//            if (swflag)){
//                
//            }
//        }
        if (n>1 && n==Eqit){
           cout<<"## at Eqit -- Et1bprev: "<<Et1bprev<<" Et1local: "<<Et1local<<" Enest "<<Enest<<endl;
        }
        ...
        bool sflag=0;
        long double origxa, origya, origza;
        if ( (ii>0 && Et1local<Enest) && (n>Eqit && (n-Eqit)%nswap==0)){
            if(Enest<0.0){
             sflag=1;
            }
        }
        ...
        if (sflag){
            ++nswaps;
            cout<<"overlap move at n "<<n<<" Et1local: "<<Et1local<<" Enest "<<Enest<<endl;
            //Swapping move
            if (nswaps==maxswaps){
                 swflag=1;
            }
            ...
            //sflag=1;
            Arandom = (unsigned int)(dsfmt_genrand_close_open(&dsfmt)*na);
            origxa = Tlocala.atom[Arandom].x;
		    origya = Tlocala.atom[Arandom].y;
			origza = Tlocala.atom[Arandom].z;
            origx = Tlocalb.atom[0].x;
			origy = Tlocalb.atom[0].y;
			origz = Tlocalb.atom[0].z;
//            if (Enest>0.0){
//                //Do swap
//                  Tlocala.atom[Arandom].x=origx;
//                Tlocala.atom[Arandom].y=origy;
//              Tlocala.atom[Arandom].z=origz;
//                Tlocalb.atom[0].x=origxa;
//              Tlocalb.atom[0].y=origya;
//             Tlocalb.atom[0].z=origza ;
//                 
//            }
//          else{
            //do overlap move
            Tlocalb.atom[0].x=origxa+ dsfmt_genrand_close_open(&dsfmt)*0.5;
            Tlocalb.atom[0].y=origya+ dsfmt_genrand_close_open(&dsfmt)*0.5;
            Tlocalb.atom[0].z=origza + dsfmt_genrand_close_open(&dsfmt)*0.5;
           // }
...
//            dEtwo=TotalEnergyTwo(Tlocala, Tlocalb)-Et1b;
		   ...
		   // Coordinate change		
			    ...
			    // Implement those moves
                    ...
                    //25 percent of N+1th particle moves are random darts    
    				if(ranp<0.05){
                        rdartflag=1;
    ...
    //			Tlocal.WrapCoordinates();
		...
		//	long double Enew = TotalEnergy(Tlocal);
            ...
            if (sflag){
                ...
                cout<<"Energy before swap was: "<<Et1b<<" after: "<<Et1local<<endl;
            }
            else{
                Et1local-=dEtwo;
            }
            //Et1local= -TotalEnergyTwo(Tlocala, Tlocalb);
            //dEtwo = -(Et1local-Et1b);
           // cout<<"Et1local "<<Et1local<<" dEtwo "<<dEtwo<<" Et1b "<<Et1b<<" -TETwo: "<<-TotalEnergyTwo(Tlocala, Tlocalb)<<endl;
            //exit(0);
			//Thermal criterion
			long double Prand = dsfmt_genrand_close_open(&dsfmt);
			long double Prat = ConfProb(dE+dEtwo, Beta);
			++ctriesloc;
//		    if (Et1b>Enest){
//		        Prat=2.0;
//		    }
...
//		    int ma=Nenest-1;
//			int mb=0;
//			for (int i = 0; i < Nenest; ++i)
//			{
//				if (Et1b<Enests[i]){
//					ma=i;
//				}
//				if (Et1local<Enests[i]){
//					mb=i;
//				}
//			}
...
//		if (swflag && (Et1local>Enest && Enest<-20.0)){
//            cout<<" Enest: "<<Enest<<" stepx "<<stepx<<" rdartflag: "<<rdartflag<<endl;
//		    cout<<"Et1local "<<Et1local<<" dEtwo "<<dEtwo<<" Et1b "<<Et1b<<" -TETwo: "<<-TotalEnergyTwo(Tlocala, Tlocalb)<<" dflag "<<dflag<<" Prat "<<Prat<<" Prand: "<<Prand<<" dE "<<dE<<" pflag "<<pflag<<" Arandom "<<Arandom<<endl;
//		}
...
//			Tlocal.ReCenterZero();
        ...
        else if(swflag && ( (Et1b>Enest && Et1local<Et1b ) ) ){
...
//			Tlocal.ReCenterZero();
		...
		else if( ii>0 && (Et1local<Enest && Prat>Prand)){
...
//			Tlocal.ReCenterZero();
		...
		if(tagg == 1){
//			    if (sflag){
//			          Tlocala.atom[Arandom].x=origxa;
//                        Tlocala.atom[Arandom].y=origya;
//                        Tlocala.atom[Arandom].z=origza;
//                        Tlocalb.atom[0].x=origx;
//                        Tlocalb.atom[0].y=origy;
//                        Tlocalb.atom[0].z=origz;
//			    }
//                else{
				    if (pflag==0){
					    Tlocala.atom[Arandom].x = origx;
...
//				long double dU = TestArea(Tlocal, scale, Eelocal);
                ...
                else if ( Et1local<Enest && Et1local>Enest-100.0){
                   // if (Et1local<Enest){
