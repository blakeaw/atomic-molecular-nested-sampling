...
int main(int argc, char *argv[])
{
	...
	int nswap = (Cit*ncut)/2;
    int maxswaps=1;
    int relaxstep = 400000;
//	boxx*=sca;
//	boxy*=sca;
//	boxz/=(sca*sca);
...
//	omp_set_num_threads(nprocs);
  ...
  //Perform markov moves on the configuration
    while(nsamp<Nsamples){
        ...
        if ( (ii>0 && Et1local<Enest) && (n>Eqit && n%nswap==0)){
            sflag=1;
        }
        if (sflag && swflag){
            sflag=0;
        ...
        if (sflag){
            ++nswaps;
            cout<<"swapping particles at n "<<n<<" Et1local: "<<Et1local<<" Enest "<<Enest<<endl;
            //Swapping move
            if (nswaps==maxswaps){
                 swflag=1;
            }
