...
int main(int argc, char *argv[])
{
	...
	const unsigned int Nsamples = 400000;
	long double NSfrac= f_frac_f;
	long double Nsmps = (long double)(Nsamples);
	int ncut = (int) ( Nsmps- NSfrac*(Nsmps+1.0) );
...
//	ofstream stepoutfile("zStepOut_int_test_a1.dat");	
	...
	unsigned int ConfigsOutFreq = ((MCitpp-Eqit))/1;
...
logfile << " " << endl;
    ...
    logfile<<" Uses diffusive NS in the style of Do et al. and moves N+1th particle N/2 times as often"<<endl;
    ...
    logfile<<"Diffusive Weight (Wd): "<<Wd<<endl;
...
long double Eemin = 100.0;
long double Elowcap = -1.0;
bool lconflag = 0;
bool zeroflag=0;
...
//put into histrogram
	Histogram Ehist;
	//Ehist.Initialize(Binwidth, Elist[0]-1.0, Elist[Nsamples-1]+1.0);
	long double Erlow=Elist[0]-1.0;
	long double Erhigh=Elist[Nsamples-1]+1.0;

	Ehist.Initialize(Binwidth, Erlow, Erhigh);
    for (int po = 0; po < Nsamples; ++po)
    {
        //push with corrected bias
      //  cout<<" E "<<Elist[po]<< " rebias "<<exp(-Beta*(-Elist[po]+scale*Elist[po]))<<endl;
        Ehist.PushToNormWithWeight(Elist[po], exp(-Beta*(-Elist[po]+scale*Elist[po])) );
    }
Ehist.Renormalize();
Enest = Ehist.IntegrateToFraction(NSfrac);
ncut=0;
for (int po = Nsamples-1; po >= 0; --po)
{
    long double Eccc = Elist[po];
    if (Eccc<Enest){
      // ++ncut; 
       break; 
    }
...
cout<<"Enest: "<<Enest<<" ncut: "<<ncut<<endl;
if (ncut==0){
    //Ehist.DumpNormal("out.hist");
//    for (int po = 0; po < Nsamples; ++po)
//    {
//        cout<<"Enest: "<<Enest<<" Elist: "<<Elist[po]<<endl;
//    }
       cout<<"Enest: "<<Enest<<" Elist: "<<Elist[Nsamples-1]<<endl;
	ncut=1;
   // exit(0);
}
}
...
long double Intavg = 0.5*(Enprev+Enest);
...
if (Eohisthigh>1000.0){
    Eohisthigh=1000.0;
}
long double Eohistbinw = (Eohisthigh-Eohistlow)/200.0;
...
if (Enest<Elowcap && !zeroflag){
	cout<<"Enest: "<<Enest<<" is less than the cap: "<<Elowcap<<endl;
    scale=0.00;
	cout<<"resetting scale to: "<<scale<<endl;
    zeroflag=1;
    ncut=Nsamples;
	Eqit=100*na;
}
//else if(Enest<-10.0 && zeroflag){
//    ncut=Nsamples;
//    scale=0.10;
//	cout<<"resetting scale to: "<<scale<<endl;
//	Eqit=100*na;
//}
//else if(Enest<-50.0 && zeroflag){
//    ncut=Nsamples;
//    scale=0.00;
//	cout<<"resetting scale to: "<<scale<<endl;
//	Eqit=100*na;
  ...
  //Perform markov moves on the configuration
    while(nsamp<Nsamples){
		...
		// randomly select an atom index	
			...
			//Arandom= (unsigned int)(dsfmt_genrand_close_open(&dsfmt)*natoms);
    ...
    //			Tlocal.WrapCoordinates();
		...
		//	long double Enew = TotalEnergy(Tlocal);
           ...
           // cout<<"Et1local "<<Et1local<<" dEtwo "<<dEtwo<<" Et1b "<<Et1b<<" -TETwo: "<<-TotalEnergyTwo(Tlocala, Tlocalb)<<endl;
			...
			long double Prat = ConfProb(dE+scale*dEtwo, Beta);
			++ctriesloc;
...
//			}
	 ...
	 if( (ii>0 && dflag) && (Prat>Prand && Et1local>Enest-200.0)){
...
//				long double dU = TestArea(Tlocal, scale, Eelocal);
				...
				long double dU = TotalEnergy(Tlocala);
				#pragma omp critical
				{
//					if (Eelocal<-10000.0){
//						Eelocal=-10000.0;
