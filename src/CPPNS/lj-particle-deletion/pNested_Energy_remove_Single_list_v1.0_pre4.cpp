...
int main(int argc, char *argv[])
{
    ...
    //long double NSfrac = 0.99500;
	long double Nsmps = (long double)(Nsamples);
	int ncut = (unsigned int) ( Nsmps +1.0 - NSfrac*(Nsmps+1.0) );
    //ncut=10;
...
logfile << " " << endl;
    ...
    //recompute the true NS fraction
    NSfrac = (Nsmps + 1.0 - (long double)(ncut))/(Nsmps+1.0);
    logfile<<"True NSfrac based on ncut is: "<<NSfrac<<endl;
    ...
    logfile<<"dartprob: "<<dartprob<<endl;
...
long double * Enests;
Enests = new long double[Iterations];
...
//                if (pflag==1){
//                    long double rpa = dsfmt_genrand_close_open(&dsfmt);
//                   if (rpa<0.250){
//                       rpa = dsfmt_genrand_close_open(&dsfmt);
//                       if (rpa<0.50){
//                            movex*=10.0;
//                            movey*=10.0;
//                            movez*=10.0;
...
//                }
			    // Implement those moves
                    ...
                    long double rprob = dsfmt_genrand_close_open(&dsfmt);
    				if(rprob<dartprob){
                        //dartprob*100 % of trial N+1th moves are random darts
