...
int main(int argc, char *argv[])
{
	...
	const unsigned short nlipids = 10000;
...
inline long double DeltaEtype1(const ConfigCGL& c, const CGLipid& moved, const CGLipid& orig){
	...
	unsigned long long Nl = c.nlipids;
		...
		long double Etoto = 0.0;
	 	#pragma omp for shared(Etotp, Etotop) private(Etot, Etoto)
...
inline long double DeltaEtype2a3(const ConfigCGL& c, const CGLipid& moved, const CGLipid& orig, unsigned short bindex){
	long double Etotp = 0.0;
	long double Etotop = 0.0;
	...
	unsigned Nl = c.nlipids;
		...
		long double Etoto = 0.0;
		//Pair Interactions
		#pragma omp sections shared(Etotp, Etotop) private(Etot, Etoto)
		#pragma omp section
		{
		for (unsigned int j = 0; j < Nl; ++j)
			{	
