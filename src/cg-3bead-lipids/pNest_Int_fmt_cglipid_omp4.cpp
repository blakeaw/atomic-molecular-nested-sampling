...
The Monte Carlo trial move consists of moving a lipid. There are three different trial moves.
...
Suggested Compiles:
-Safe Compile
 c++ -O2 source.cpp -o binname
-Execution Speed Increase
 c++ -O2 -ffast-math -fcx-fortran-rules source.cpp -o binname
...
#include "/net/uu/nm/cm/bxw109120/NestedSampling/Lipid/src/class/HistogramNS_slim.h"
...
int main(int argc, char *argv[])
{
	...
	const unsigned short nlipids = 2;
...
//	const long double box = (1.250*(long double)(nlipids));
	long double box;
	if(nlipids<=20){
		box = (1.250*(long double)(nlipids))*7.6;
	}
	else if(nlipids>20 && nlipids<=50){
		box = pow(0.976, nlipids)*(long double)(nlipids)*7.6;
	}
	else{
		box = pow(0.988, nlipids)*(long double)(nlipids)*7.6;
	}
	...
	const unsigned long long nbins = 8000000;
	const long double Elow = (-5000.0)*(long double)(nlipids);
	long double Ehigh = 10.0;
	const unsigned collectstepguess = nlipids*5;
	unsigned collectstep = collectstepguess;
	//Output Files
	string rundescript = "cube_l2_test_bb";
	//Name of logfile-
	...
	const unsigned int ConfigsOutFreq = (MCit/nprocs)-1; 
...
//----End of Parameter setting-----
   ...
   //Initialize output file streams
	...
	const unsigned long long seed = time(0);
	...
	cout<<"Assigning Initial Configuration with energy less than: "<<Ehigh<<" ...."<<endl;
...
//	unsigned long long MCitpp = MCit;
	...
	{
	ConfigCGL Tlocal;
	Tlocal.InitializeLipids(nlipids);
	Tlocal.SetBox(box, box, box);
	Tlocal.EquateCoord(confn);
	long double Eetloc;
	long double Etloc=Et1;	
	long double percentlocal=0.0;
	unsigned short th_id;
	th_id = omp_get_thread_num();
//	#pragma omp parallel for private(Eetloc, Etloc, percentlocal, Tlocal)
	...
	//Trial Moves
		...
		unsigned Lrandom = (unsigned)(dsfmt_genrand_close_open(&dsfmt)*nlipids);
		//Assign a storage Lipid to save current coordinates
		CGLipid OClip;
		OClip.EquateLipidCoord(Tlocal.lipid[Lrandom]);
		...
		/* set random movements of the coordinates
		   of the selected atom */
			...
			{
				Tlocal.lipid[Lrandom].bead[b].x += movex;
				Tlocal.lipid[Lrandom].bead[b].y += movey;
				Tlocal.lipid[Lrandom].bead[b].z += movez;
			}
			dE = DeltaEtype1(Tlocal, Tlocal.lipid[Lrandom], OClip);
		}
		/* Second type: ttype = 1; random rotation
			...
			RandomRotation(u, v, stepa, Tlocal.lipid[Lrandom], ranrindex);
			dE = DeltaEtype2a3(Tlocal, Tlocal.lipid[Lrandom], OClip, ranrindex);
		...
		/* set random movements of the coordinates
		   of the selected atom */
			movex = (dsfmt_genrand_close_open(&dsfmt)-0.5)*step*0.5;
			movey = (dsfmt_genrand_close_open(&dsfmt)-0.5)*step*0.5;
			movez = (dsfmt_genrand_close_open(&dsfmt)-0.5)*step*0.5;
			// Implement those moves
			unsigned short rb = (unsigned short)(Tlocal.lipid[Lrandom].nbeads*dsfmt_genrand_close_open(&dsfmt));
			
				Tlocal.lipid[Lrandom].bead[rb].x += movex;
				Tlocal.lipid[Lrandom].bead[rb].y += movey;
				Tlocal.lipid[Lrandom].bead[rb].z += movez;
				//cout<<"rb "<<rb<<endl;
			dE =DeltaEtype2a3(Tlocal, Tlocal.lipid[Lrandom], OClip, rb);
			/*Type 2 move sometimes returns nan for dE
				if this happens we need to reset dE to 
				a large value to cause the move to fail.
			*/
			if(dE != dE){
			 //cout<<"-- dE is nan. Resetting..."<<endl;
			 dE=1000000.0;
			}
		}
		//if(dE != dE){
		...
		//cout<<"Etloc "<<Etloc<<" dE "<<dE<<" ttype "<<ttype<<endl;
...
//		bool dccheck = confn.BoxCheck();
		bool dccheck = Tlocal.BoxCheck(Lrandom);
	...
	//	cout<<"Thread "<<th_id<<" In critical"<<endl;
	//	cout<<"----- Eetloc: "<<Eetloc<<" Etloc: "<<Etloc<<endl;
		if(tagg == 1){
			
			Tlocal.lipid[Lrandom].EquateLipidCoord(OClip);
			if(hardsuccess == 1){
				Etloc=Et1s;
	...
	//	cout << " Et1: " << Et1 << endl;
		...
		//		cout<<"-------Pushing energy: "<<Eetloc<<" at step "<<j<<" Emed is "<<Emedian<<endl;		
				//if(Eetloc>Emedian){
		...
		if(j>Eqit && j%ConfigsOutFreq ==0){
			...
			coordsfile << "coarse grained lipids with Energy " << Eetloc << endl;
			for(unsigned long long hg = 0; hg<nbeads; hg++){
				unsigned long Di = hg/3;
				unsigned long long Ai = hg - (3*Di);
		
				coordsfile << setiosflags(ios::fixed) << setprecision(15) << Tlocal.lipid[Di].bead[Ai].type << " " << Tlocal.lipid[Di].bead[Ai].x << " " << Tlocal.lipid[Di].bead[Ai].y << " " << Tlocal.lipid[Di].bead[Ai].z << endl; 
			}
		}
...
//		cout << "percentlocal: "<<percentlocal<<endl;
...
//Calc percent
percent /= (long double)(MCitpp*nprocs)/100.0;
		...
		for (unsigned int j = i+1; j < Nl; ++j)
		{
			...
			for (unsigned int b = 0; b < Nb; ++b)
			{
				...
				for (unsigned int p = 0; p < Nb2; ++p)
				{
					...
					Etot += PairCGLipid(c.lipid[i].bead[b], c.lipid[j].bead[p], r);
				}
			}
		}		
...
long double PairCGLipid(const Bead& one, const Bead& two, long double r){
	...
	long double E = 0.0;
	const long double t1 = one.type;
	const long double t2 = two.type;
	long double b, e;
	const long double wc = 1.6*(one.molec->sigma);
	long double rc;
	bool itag;
...
long double DeltaEtype1(const ConfigCGL& c, const CGLipid& moved, const CGLipid& orig){
	...
	long double r;
	long double ro;
	const unsigned mindex = moved.index;	
	
	const unsigned long long Nl = c.nlipids;
...
void RandomRotation(const long double& u, const long double& v, const long double& stepa, CGLipid& L, const unsigned long& Br2){
		...
		const unsigned long long Br1= 1;
